...
**lease_name** | **str** | LeaseName is the name of the lease for which this candidate is contending. The limits on this field are the same as on Lease.name. Multiple lease candidates may reference the same Lease.name. This field is immutable. | [default to '']
...
```python
from kubernetes_asyncio_pydantic.models.v1beta1_lease_candidate_spec import V1beta1LeaseCandidateSpec
