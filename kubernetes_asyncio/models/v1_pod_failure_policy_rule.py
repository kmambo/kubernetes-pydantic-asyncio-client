...
class V1PodFailurePolicyRule(BaseModel):
    ...
    on_exit_codes: Optional[V1PodFailurePolicyOnExitCodesRequirement] = Field(
        default=None,
        description="Represents the requirement on the container exit codes.",
        alias="onExitCodes",
    )
    on_pod_conditions: Optional[List[V1PodFailurePolicyOnPodConditionsPattern]] = Field(
        default=None,
        ...
        _obj = cls.model_validate(
            {
                ...
                "onExitCodes": (
                    V1PodFailurePolicyOnExitCodesRequirement.from_dict(
                        obj["onExitCodes"]
                    )
                    if obj.get("onExitCodes") is not None
                    else None
                ),
                "onPodConditions": (
                    [
                        V1PodFailurePolicyOnPodConditionsPattern.from_dict(_item)
                        for _item in obj["onPodConditions"]
                    ]
                    if obj.get("onPodConditions") is not None
                    else None
                ),
            }
        )
