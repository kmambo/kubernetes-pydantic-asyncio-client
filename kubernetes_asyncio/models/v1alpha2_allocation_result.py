...
class V1alpha2AllocationResult(BaseModel):
    ...
    available_on_nodes: Optional[V1NodeSelector] = Field(
        default=None,
        description="This field will get set by the resource driver after it has allocated the resource to inform the scheduler where it can schedule Pods using the ResourceClaim.  Setting this field is optional. If null, the resource is available everywhere.",
        alias="availableOnNodes",
    )
    resource_handles: Optional[List[V1alpha2ResourceHandle]] = Field(
        default=None,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.resource_handles:
            for _item_resource_handles in self.resource_handles:
                if _item_resource_handles:
                    _items.append(_item_resource_handles.to_dict())
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "availableOnNodes": (
                    V1NodeSelector.from_dict(obj["availableOnNodes"])
                    if obj.get("availableOnNodes") is not None
                    else None
                ),
                "resourceHandles": (
                    [
                        V1alpha2ResourceHandle.from_dict(_item)
                        for _item in obj["resourceHandles"]
                    ]
                    if obj.get("resourceHandles") is not None
                    else None
                ),
