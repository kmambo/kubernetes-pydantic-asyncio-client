...
class V1alpha2ResourceClaimSpec(BaseModel):
    ...
    parameters_ref: Optional[V1alpha2ResourceClaimParametersReference] = Field(
        default=None,
        description="ParametersRef references a separate object with arbitrary parameters that will be used by the driver when allocating a resource for the claim.  The object must be in the same namespace as the ResourceClaim.",
        alias="parametersRef",
    )
    resource_class_name: StrictStr = Field(
        ...
        _obj = cls.model_validate(
            {
                ...
                "resourceClassName": (
                    obj.get("resourceClassName")
                    if obj.get("resourceClassName") is not None
                    else ""
                ),
            }
        )
