...
class V1EndpointSubset(BaseModel):
    ...
    addresses: Optional[List[V1EndpointAddress]] = Field(
        default=None,
        description="IP addresses which offer the related ports that are marked as ready. These endpoints should be considered safe for load balancers and clients to utilize.",
    )
    not_ready_addresses: Optional[List[V1EndpointAddress]] = Field(
        default=None,
        description="IP addresses which offer the related ports but are not currently marked as ready because they have not yet finished starting, have recently failed a readiness check, or have recently failed a liveness check.",
        alias="notReadyAddresses",
    )
    ports: Optional[List[V1EndpointPort]] = Field(
        default=None, description="Port numbers available on the related IP addresses."
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "addresses": (
                    [V1EndpointAddress.from_dict(_item) for _item in obj["addresses"]]
                    if obj.get("addresses") is not None
                    else None
                ),
                "notReadyAddresses": (
                    [
                        V1EndpointAddress.from_dict(_item)
                        for _item in obj["notReadyAddresses"]
                    ]
                    if obj.get("notReadyAddresses") is not None
                    else None
                ),
                "ports": (
                    [V1EndpointPort.from_dict(_item) for _item in obj["ports"]]
