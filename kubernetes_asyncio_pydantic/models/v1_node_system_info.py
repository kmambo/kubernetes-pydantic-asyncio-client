...
class V1NodeSystemInfo(BaseModel):
    ...
    boot_id: StrictStr = Field(
        description="Boot ID reported by the node.", alias="bootID"
    )
    container_runtime_version: StrictStr = Field(
    ...
    machine_id: StrictStr = Field(
        description="MachineID reported by the node. For unique machine identification in the cluster this field is preferred. Learn more from man(5) machine-id: http://man7.org/linux/man-pages/man5/machine-id.5.html",
        alias="machineID",
    )
    operating_system: StrictStr = Field(
        description="The Operating System reported by the node", alias="operatingSystem"
    )
        ...
        _obj = cls.model_validate(
            {
                "architecture": (
                    ...
                    if obj.get("architecture") is not None
                    else ""
                ),
                "bootID": obj.get("bootID") if obj.get("bootID") is not None else "",
                "containerRuntimeVersion": (
                    obj.get("containerRuntimeVersion")
                    if obj.get("containerRuntimeVersion") is not None
                    else ""
                ),
                "kernelVersion": (
                    obj.get("kernelVersion")
                    if obj.get("kernelVersion") is not None
                    else ""
                ),
                "kubeProxyVersion": (
                    obj.get("kubeProxyVersion")
                    if obj.get("kubeProxyVersion") is not None
                    else ""
                ),
                "kubeletVersion": (
                    obj.get("kubeletVersion")
                    if obj.get("kubeletVersion") is not None
                    else ""
                ),
                "machineID": (
                    obj.get("machineID") if obj.get("machineID") is not None else ""
                ),
                "operatingSystem": (
                    obj.get("operatingSystem")
                    if obj.get("operatingSystem") is not None
                    else ""
                ),
