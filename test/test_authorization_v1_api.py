# coding: utf-8

"""
    Kubernetes

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: master
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from kubernetes_asyncio.client.api.authorization_v1_api import AuthorizationV1Api


class TestAuthorizationV1Api(unittest.IsolatedAsyncioTestCase):
    """AuthorizationV1Api unit test stubs"""

    async def asyncSetUp(self) -> None:
        self.api = AuthorizationV1Api()

    async def asyncTearDown(self) -> None:
        await self.api.api_client.close()

    async def test_create_namespaced_local_subject_access_review(self) -> None:
        """Test case for create_namespaced_local_subject_access_review

        """
        pass

    async def test_create_self_subject_access_review(self) -> None:
        """Test case for create_self_subject_access_review

        """
        pass

    async def test_create_self_subject_rules_review(self) -> None:
        """Test case for create_self_subject_rules_review

        """
        pass

    async def test_create_subject_access_review(self) -> None:
        """Test case for create_subject_access_review

        """
        pass

    async def test_get_api_resources(self) -> None:
        """Test case for get_api_resources

        """
        pass


if __name__ == '__main__':
    unittest.main()
