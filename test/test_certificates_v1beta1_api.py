# coding: utf-8

"""
    Kubernetes

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: master
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from kubernetes_asyncio.client.api.certificates_v1beta1_api import CertificatesV1beta1Api


class TestCertificatesV1beta1Api(unittest.IsolatedAsyncioTestCase):
    """CertificatesV1beta1Api unit test stubs"""

    async def asyncSetUp(self) -> None:
        self.api = CertificatesV1beta1Api()

    async def asyncTearDown(self) -> None:
        await self.api.api_client.close()

    async def test_create_cluster_trust_bundle(self) -> None:
        """Test case for create_cluster_trust_bundle

        """
        pass

    async def test_delete_cluster_trust_bundle(self) -> None:
        """Test case for delete_cluster_trust_bundle

        """
        pass

    async def test_delete_collection_cluster_trust_bundle(self) -> None:
        """Test case for delete_collection_cluster_trust_bundle

        """
        pass

    async def test_get_api_resources(self) -> None:
        """Test case for get_api_resources

        """
        pass

    async def test_list_cluster_trust_bundle(self) -> None:
        """Test case for list_cluster_trust_bundle

        """
        pass

    async def test_patch_cluster_trust_bundle(self) -> None:
        """Test case for patch_cluster_trust_bundle

        """
        pass

    async def test_read_cluster_trust_bundle(self) -> None:
        """Test case for read_cluster_trust_bundle

        """
        pass

    async def test_replace_cluster_trust_bundle(self) -> None:
        """Test case for replace_cluster_trust_bundle

        """
        pass


if __name__ == '__main__':
    unittest.main()
