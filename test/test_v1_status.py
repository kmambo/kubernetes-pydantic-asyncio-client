# coding: utf-8

"""
    merged spec

    merged spec

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from kubernetes_asyncio_pydantic.models.v1_status import V1Status

class TestV1Status(unittest.TestCase):
    """V1Status unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1Status:
        """Test V1Status
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1Status`
        """
        model = V1Status()
        if include_optional:
            return V1Status(
                api_version = '',
                code = 56,
                details = kubernetes_asyncio_pydantic.models.v1/status_details.v1.StatusDetails(
                    causes = [
                        null
                        ], 
                    group = '', 
                    kind = '', 
                    name = '', 
                    retry_after_seconds = 56, 
                    uid = '', ),
                kind = '',
                message = '',
                metadata = kubernetes_asyncio_pydantic.models.v1/list_meta.v1.ListMeta(
                    continue = '', 
                    remaining_item_count = 56, 
                    resource_version = '', 
                    self_link = '', ),
                reason = '',
                status = ''
            )
        else:
            return V1Status(
        )
        """

    def testV1Status(self):
        """Test V1Status"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
