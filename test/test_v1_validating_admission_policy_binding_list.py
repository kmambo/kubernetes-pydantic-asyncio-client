# coding: utf-8

"""
    Kubernetes

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: master
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from kubernetes_asyncio.client.models.v1_validating_admission_policy_binding_list import V1ValidatingAdmissionPolicyBindingList

class TestV1ValidatingAdmissionPolicyBindingList(unittest.TestCase):
    """V1ValidatingAdmissionPolicyBindingList unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1ValidatingAdmissionPolicyBindingList:
        """Test V1ValidatingAdmissionPolicyBindingList
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1ValidatingAdmissionPolicyBindingList`
        """
        model = V1ValidatingAdmissionPolicyBindingList()
        if include_optional:
            return V1ValidatingAdmissionPolicyBindingList(
                api_version = '',
                items = [
                    kubernetes_asyncio.client.models.v1/validating_admission_policy_binding.v1.ValidatingAdmissionPolicyBinding(
                        api_version = '', 
                        kind = '', 
                        metadata = kubernetes_asyncio.client.models.v1/object_meta.v1.ObjectMeta(
                            annotations = {
                                'key' : ''
                                }, 
                            creation_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            deletion_grace_period_seconds = 56, 
                            deletion_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            finalizers = [
                                ''
                                ], 
                            generate_name = '', 
                            generation = 56, 
                            labels = {
                                'key' : ''
                                }, 
                            managed_fields = [
                                kubernetes_asyncio.client.models.v1/managed_fields_entry.v1.ManagedFieldsEntry(
                                    api_version = '', 
                                    fields_type = '', 
                                    fields_v1 = kubernetes_asyncio.client.models.fields_v1.fieldsV1(), 
                                    manager = '', 
                                    operation = '', 
                                    subresource = '', 
                                    time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), )
                                ], 
                            name = '', 
                            namespace = '', 
                            owner_references = [
                                kubernetes_asyncio.client.models.v1/owner_reference.v1.OwnerReference(
                                    api_version = '', 
                                    block_owner_deletion = True, 
                                    controller = True, 
                                    kind = '', 
                                    name = '', 
                                    uid = '', )
                                ], 
                            resource_version = '', 
                            self_link = '', 
                            uid = '', ), 
                        spec = kubernetes_asyncio.client.models.v1/validating_admission_policy_binding_spec.v1.ValidatingAdmissionPolicyBindingSpec(
                            match_resources = kubernetes_asyncio.client.models.v1/match_resources.v1.MatchResources(
                                exclude_resource_rules = [
                                    kubernetes_asyncio.client.models.v1/named_rule_with_operations.v1.NamedRuleWithOperations(
                                        api_groups = [
                                            ''
                                            ], 
                                        api_versions = [
                                            ''
                                            ], 
                                        operations = [
                                            ''
                                            ], 
                                        resource_names = [
                                            ''
                                            ], 
                                        resources = [
                                            ''
                                            ], 
                                        scope = '', )
                                    ], 
                                match_policy = '', 
                                namespace_selector = kubernetes_asyncio.client.models.v1/label_selector.v1.LabelSelector(
                                    match_expressions = [
                                        kubernetes_asyncio.client.models.v1/label_selector_requirement.v1.LabelSelectorRequirement(
                                            key = '', 
                                            operator = '', 
                                            values = [
                                                ''
                                                ], )
                                        ], 
                                    match_labels = {
                                        'key' : ''
                                        }, ), 
                                object_selector = kubernetes_asyncio.client.models.v1/label_selector.v1.LabelSelector(), 
                                resource_rules = [
                                    kubernetes_asyncio.client.models.v1/named_rule_with_operations.v1.NamedRuleWithOperations(
                                        scope = '', )
                                    ], ), 
                            param_ref = kubernetes_asyncio.client.models.v1/param_ref.v1.ParamRef(
                                name = '', 
                                namespace = '', 
                                parameter_not_found_action = '', 
                                selector = , ), 
                            policy_name = '', 
                            validation_actions = [
                                ''
                                ], ), )
                    ],
                kind = '',
                metadata = kubernetes_asyncio.client.models.v1/list_meta.v1.ListMeta(
                    continue = '', 
                    remaining_item_count = 56, 
                    resource_version = '', 
                    self_link = '', )
            )
        else:
            return V1ValidatingAdmissionPolicyBindingList(
                items = [
                    kubernetes_asyncio.client.models.v1/validating_admission_policy_binding.v1.ValidatingAdmissionPolicyBinding(
                        api_version = '', 
                        kind = '', 
                        metadata = kubernetes_asyncio.client.models.v1/object_meta.v1.ObjectMeta(
                            annotations = {
                                'key' : ''
                                }, 
                            creation_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            deletion_grace_period_seconds = 56, 
                            deletion_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            finalizers = [
                                ''
                                ], 
                            generate_name = '', 
                            generation = 56, 
                            labels = {
                                'key' : ''
                                }, 
                            managed_fields = [
                                kubernetes_asyncio.client.models.v1/managed_fields_entry.v1.ManagedFieldsEntry(
                                    api_version = '', 
                                    fields_type = '', 
                                    fields_v1 = kubernetes_asyncio.client.models.fields_v1.fieldsV1(), 
                                    manager = '', 
                                    operation = '', 
                                    subresource = '', 
                                    time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), )
                                ], 
                            name = '', 
                            namespace = '', 
                            owner_references = [
                                kubernetes_asyncio.client.models.v1/owner_reference.v1.OwnerReference(
                                    api_version = '', 
                                    block_owner_deletion = True, 
                                    controller = True, 
                                    kind = '', 
                                    name = '', 
                                    uid = '', )
                                ], 
                            resource_version = '', 
                            self_link = '', 
                            uid = '', ), 
                        spec = kubernetes_asyncio.client.models.v1/validating_admission_policy_binding_spec.v1.ValidatingAdmissionPolicyBindingSpec(
                            match_resources = kubernetes_asyncio.client.models.v1/match_resources.v1.MatchResources(
                                exclude_resource_rules = [
                                    kubernetes_asyncio.client.models.v1/named_rule_with_operations.v1.NamedRuleWithOperations(
                                        api_groups = [
                                            ''
                                            ], 
                                        api_versions = [
                                            ''
                                            ], 
                                        operations = [
                                            ''
                                            ], 
                                        resource_names = [
                                            ''
                                            ], 
                                        resources = [
                                            ''
                                            ], 
                                        scope = '', )
                                    ], 
                                match_policy = '', 
                                namespace_selector = kubernetes_asyncio.client.models.v1/label_selector.v1.LabelSelector(
                                    match_expressions = [
                                        kubernetes_asyncio.client.models.v1/label_selector_requirement.v1.LabelSelectorRequirement(
                                            key = '', 
                                            operator = '', 
                                            values = [
                                                ''
                                                ], )
                                        ], 
                                    match_labels = {
                                        'key' : ''
                                        }, ), 
                                object_selector = kubernetes_asyncio.client.models.v1/label_selector.v1.LabelSelector(), 
                                resource_rules = [
                                    kubernetes_asyncio.client.models.v1/named_rule_with_operations.v1.NamedRuleWithOperations(
                                        scope = '', )
                                    ], ), 
                            param_ref = kubernetes_asyncio.client.models.v1/param_ref.v1.ParamRef(
                                name = '', 
                                namespace = '', 
                                parameter_not_found_action = '', 
                                selector = , ), 
                            policy_name = '', 
                            validation_actions = [
                                ''
                                ], ), )
                    ],
        )
        """

    def testV1ValidatingAdmissionPolicyBindingList(self):
        """Test V1ValidatingAdmissionPolicyBindingList"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
